...
vector<ThreatsObject*> MakeThreatsList() {// all mối hiểm họa
	...
	ThreatsObject* dynamic_threats = new ThreatsObject[20];//mảng chứa 40 mối hiểm họa di chuyển được
	for (int i = 0; i < 20; i++) {
		...
		if (p_threats != NULL) {
			...
			p_threats->set_x_pos(500 + i * 500);//vị trí threats
	...
	ThreatsObject* threats_object_ = new ThreatsObject[20];// mảng chứa 20 mối hiểm họa
	for (int i = 0; i < 20; i++) {
		ThreatsObject* p_threats = (threats_object_ + i);
		if (p_threats != NULL) {
			p_threats->LoadImg("img//threat_bullet.png", g_screen);
			p_threats->set_clips();
			p_threats->set_x_pos(700 + i * 1200);//vị trí threats
